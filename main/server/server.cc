...
server::server(
		std::string& port,
		const std::uint16_t fifo_size,
		const std::uint16_t fifo_low_watermark,
		const std::uint16_t fifo_high_watermark,
		const std::uint16_t buf_len,
		const std::uint16_t tx_interval
		)
	:	
		io_service_(),
		...
		mixer_buf_(), // Potem się go jakoś wypełni.
		port_(port),
...
void server::do_manage_msg(base_header* header, std::string& body)
{
	...
	boost::asio::ip::udp::endpoint remote_endpoint(sender_endpoint_);
	if (header_name == CLIENT) {
		...
		std::uint32_t clientid = c_header->_client_id;
		// Zapisz endpoint do stringa.
		const std::string client_str_udp_endpoint =
			convert_remote_udp_endpoint_to_string(remote_endpoint);
		...
		const size_t client_id = static_cast<size_t>(
			client_map_[remote_endpoint]
		);
		// Ściągnij nr, od którego klient prosi o retransmisję.
		std::uint32_t retransmit_inf_nr = r_header->_nr;
		...
		// Ściągnij identyfikator klienta.
		const size_t clientid = client_map_[remote_endpoint];
		// ZAKŁADAMY PÓKI CO, ŻE PRZESŁANY NUMER JEST DOBRY. TODO: Sprawdzić, czy tak faktycznie jest.
		const std::uint32_t client_dgram_nr = u_header->_nr;
		...
		if (decision) {
			...
			const std::uint16_t next_ack = client_dgram_nr + 1;
			// Rozmiar kolejki.
...
void server::mixer()
{
	...
	std::list<std::uint32_t> active_queue_clients_id;
	...
	size_t output_size = CLIENT_BUFFER_LEN;
	...
	for(auto it = client_data_map_.begin(); 
		...
		const std::uint32_t clientid = it->first;
		const std::uint32_t cnr = static_cast<uint32_t>(current_datagram_nr_);

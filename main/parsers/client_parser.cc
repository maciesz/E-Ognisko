...
response client_parser::parse(boost::uint16_t& port, std::string& server_name, 
		boost::uint16_t& retransmit, const program_parametres& prms)
{
	...
	try {
		...
		desc.add_options()
			...
			(",X", po::value<boost::uint16_t>(&retransmit), "RETRANSMIT_LIMIT")
		;
